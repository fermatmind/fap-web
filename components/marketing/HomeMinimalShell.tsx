...
export function HomeMinimalShell({ locale }: { locale: Locale }) {
  const isZh = locale === "zh";

  return (
    <main className="fm-page-background min-h-screen text-[var(--fm-text)]" data-testid="home-minimal-shell">
      <Container className="flex min-h-screen max-w-3xl flex-col justify-center gap-6 py-16">
        ...
        <h1 className="m-0 font-serif text-3xl font-semibold leading-tight md:text-5xl">
          {isZh ? "内容暂时不可用" : "Content is temporarily unavailable"}
        </h1>
        <p className="m-0 max-w-2xl text-base leading-7 text-[var(--fm-text-muted)]">
          {isZh
            ? "你仍然可以继续进入核心测评入口。"
            : "You can still continue to the core assessment entry points."}
        </p>

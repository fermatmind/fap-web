...
export function CareerDossierWorkRisk({ value, locale, sectionLabel, sectionLabelId, interfaceLabels }: DecisionJourneyProps<CareerPublishedWorkRisk>) {
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2" data-career-api-list="career_risk_cards.risks">
          {value.risks.map((risk, index) => (
            <article key={risk.id} className="rounded-xl border border-[#DCE3F0] bg-[#F8FAFD] p-5">
              ...
              {[
                [(interfaceLabels?.["interface.risk.scenario_label"] ?? (isZh ? "典型场景" : "Typical scenario")), risk.scenario, "scenario"],
                [(interfaceLabels?.["interface.risk.affected_roles_label"] ?? (isZh ? "主要岗位" : "Roles most affected")), risk.affected_roles, "affected_roles"],
                [(interfaceLabels?.["interface.risk.consequence_label"] ?? (isZh ? "可能后果" : "Possible consequence")), risk.consequence, "consequence"],
                [(interfaceLabels?.["interface.risk.mitigation_label"] ?? (isZh ? "降低风险" : "How to reduce risk")), risk.mitigation, "mitigation"],
              ...
              <p className="m-0 mt-3 text-xs text-[#657087]">{(interfaceLabels?.["interface.risk.evidence_label"] ?? (isZh ? "依据" : "Evidence"))}：{risk.evidence_refs.map((id) => sources.get(id)?.label ?? id).join(" · ")}</p>
            </article>
          ))}
        </div>
    ...
    <div className="min-w-0 bg-white" data-testid="career-dossier-progression" data-career-api-component="career_path_block">
      ...
      <div className="px-6 py-7 sm:px-8">
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-4">
                {track.stages.map((stage, stageIndex) => (
                  <article key={stage.role} className="relative rounded-xl border border-[#DCE3F0] bg-[#F8FAFD] p-4 pt-10">
                    ...
                    <p className="m-0 mt-3 text-sm leading-6 text-[#465066]"><strong>{(interfaceLabels?.["interface.path.responsibilities_label"] ?? (isZh ? "典型职责" : "Typical scope"))}：</strong>{stage.responsibility}</p>
                    <p className="m-0 mt-3 text-sm leading-6 text-[#465066]"><strong>{(interfaceLabels?.["interface.path.promotion_evidence_label"] ?? (isZh ? "晋升证据" : "Readiness evidence"))}：</strong>{stage.readiness_evidence}</p>
                    <p className="m-0 mt-3 text-sm leading-6 text-[#465066]"><strong>{(interfaceLabels?.["interface.path.credential_boundary_label"] ?? (isZh ? "资格边界" : "Credential boundary"))}：</strong>{stage.credentials}</p>
                    <p className="m-0 mt-3 text-sm leading-6 text-[#465066]"><strong>{(interfaceLabels?.["interface.path.next_step_label"] ?? (isZh ? "下一步" : "Next moves"))}：</strong>{stage.next_moves}</p>
                  </article>
                ))}
              </div>
            </section>
        ...
        <section className="mt-8 border-t border-[#E5E9F2] pt-7" aria-labelledby="career-competence-ladder-title">
          <h3 id="career-competence-ladder-title" className="m-0 text-xl font-bold text-[#172A60]">{(interfaceLabels?.["interface.path.competence_ladder_heading"] ?? (isZh ? "能力阶梯：晋升看什么证据？" : "Capability ladder: what demonstrates readiness?"))}</h3>
...
export function CareerDossierOutlookTransitions({
  ...
  return (
    <div className="min-w-0 bg-white" data-testid="career-dossier-outlook-transitions" data-career-api-component="market_signal_card">
      ...
      <div className="px-6 py-7 sm:px-8">
        <section aria-labelledby="career-outlook-evidence-title"><h3 id="career-outlook-evidence-title" className="m-0 text-xl font-bold text-[#172A60]">{(interfaceLabels?.["interface.outlook.evidence_heading"] ?? (isZh ? "三类数据，三种不同口径" : "Three sources, three different measures"))}</h3>
          <div className="mt-4 grid gap-4 lg:grid-cols-3" data-career-api-list="market_signal_card.outlook_evidence">{value.outlook_evidence.map((item, index) => {
            ...
            return <article key={`${item.source_id ?? item.geography}:${item.occupation_scope}:${index}`} className="rounded-xl border border-[#DCE3F0] bg-[#F8FAFD] p-5"><div className="flex items-start justify-between gap-3"><h4 className="m-0 text-base font-bold text-[#243B7A]">{title}</h4><span className="rounded-full bg-[#E9EEFB] px-3 py-1 text-xs font-bold text-[#2C3E8C]">{item.horizon}</span></div><p className="m-0 mt-3 text-sm text-[#5B6678]">{item.geography} · {item.occupation_scope}</p><p className="m-0 mt-4 text-2xl font-bold text-[#172A60]" {...apiField(`market_signal_card.outlook_evidence[${index}].value`)}>{item.value}</p><p className="m-0 mt-1 text-xs font-bold uppercase tracking-wide text-[#0E9F94]">{item.metric}</p><p className="m-0 mt-4 text-sm leading-6 text-[#465066]">{item.interpretation}</p><p className="m-0 mt-3 rounded-lg bg-[#FFF6E9] p-3 text-xs leading-5 text-[#6A5738]"><strong>{(interfaceLabels?.["interface.outlook.evidence_limitation_label"] ?? (isZh ? "不能说明" : "Does not establish"))}：</strong>{item.limitation}</p><CareerEvidenceLine content={contentV3} factRefs={item.fact_ref ? [item.fact_ref] : []} sourceRefs={item.source_ref ? [item.source_ref] : []} /></article>;
          })}</div>
        ...
        <section className="mt-8 border-t border-[#E5E9F2] pt-7" aria-labelledby="career-transitions-title"><h3 id="career-transitions-title" className="m-0 text-xl font-bold text-[#172A60]">{(interfaceLabels?.["interface.outlook.transitions_heading"] ?? (isZh ? "现有能力可以迁移到哪里？" : "Where can your current capabilities transfer?"))}</h3><p className="m-0 mt-2 text-sm leading-6 text-[#5B6678]">{(interfaceLabels?.["interface.outlook.transitions_intro"] ?? (isZh ? "这些是能力邻近关系，不代表自动晋升或无需补课。" : "These are capability relationships, not automatic promotions or credential waivers."))}</p>
          <div className="mt-4 grid gap-4 md:grid-cols-2" data-career-api-list="market_signal_card.transitions">{value.transitions.map((item, index) => <Link key={item.target_slug} href={item.target_href} className="group rounded-xl border border-[#DCE3F0] bg-[#F8FAFD] p-5 no-underline transition hover:-translate-y-0.5 hover:border-[#2C3E8C] hover:shadow-md" data-career-transition-target={item.target_slug}><div className="flex items-start justify-between gap-3"><h4 className="m-0 text-base font-bold text-[#243B7A]" {...apiField(`market_signal_card.transitions[${index}].target_title`)}>{item.target_title}</h4><span className="shrink-0 rounded-full bg-[#E9EEFB] px-3 py-1 text-xs font-bold text-[#2C3E8C]">{item.transition_distance}</span></div><p className="m-0 mt-3 text-sm leading-6 text-[#465066]"><strong>{(interfaceLabels?.["interface.outlook.shared_capabilities_label"] ?? (isZh ? "可迁移" : "Transfers"))}：</strong>{item.shared_capabilities}</p><p className="m-0 mt-2 text-sm leading-6 text-[#465066]"><strong>{(interfaceLabels?.["interface.outlook.capability_gaps_label"] ?? (isZh ? "需补足" : "Build next"))}：</strong>{item.capability_gaps}</p><span className="mt-4 inline-flex font-bold text-[#2C3E8C]">{(interfaceLabels?.["interface.outlook.detail_link_label"] ?? (isZh ? "查看职业详情" : "View career profile"))} <span aria-hidden="true" className="ml-1 transition group-hover:translate-x-1">→</span></span></Link>)}</div>
        </section>
      </div>

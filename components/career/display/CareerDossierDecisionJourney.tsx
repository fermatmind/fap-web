    ...
    <div className="min-w-0 bg-white" data-testid="career-dossier-outlook-transitions" data-career-api-component="market_signal_card">
      ...
      <div className="px-6 py-7 sm:px-8">
        <section aria-labelledby="career-outlook-evidence-title"><h3 id="career-outlook-evidence-title" className="m-0 text-xl font-bold text-[#172A60]">{isZh ? "三类数据，三种不同口径" : "Three sources, three different measures"}</h3>
          <div className="mt-4 grid gap-4 lg:grid-cols-3" data-career-api-list="market_signal_card.outlook_evidence">{value.outlook_evidence.map((item, index) => <article key={item.source_id} className="rounded-xl border border-[#DCE3F0] bg-[#F8FAFD] p-5"><div className="flex items-start justify-between gap-3"><h4 className="m-0 text-base font-bold text-[#243B7A]">{sources.get(item.source_id)?.label ?? item.source_id}</h4><span className="rounded-full bg-[#E9EEFB] px-3 py-1 text-xs font-bold text-[#2C3E8C]">{item.horizon}</span></div><p className="m-0 mt-3 text-sm text-[#5B6678]">{item.geography} · {item.occupation_scope}</p><p className="m-0 mt-4 text-2xl font-bold text-[#172A60]" {...apiField(`market_signal_card.outlook_evidence[${index}].value`)}>{item.value}</p><p className="m-0 mt-1 text-xs font-bold uppercase tracking-wide text-[#0E9F94]">{item.metric}</p><p className="m-0 mt-4 text-sm leading-6 text-[#465066]">{item.interpretation}</p><p className="m-0 mt-3 rounded-lg bg-[#FFF6E9] p-3 text-xs leading-5 text-[#6A5738]"><strong>{isZh ? "不能说明" : "Does not establish"}：</strong>{item.limitation}</p></article>)}</div>

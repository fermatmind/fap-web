...
The Window 2 M01 package supplies the joint Search Analytics evidence:
...
- manifest: `window-02-measurement/m01-gsc-export/m01_export_manifest.json`
...
E01 retains 4,192 rows across 152 verified English canonicals after applying the current route contract. It excludes four observed Big Five `high-*` / `low-*` redirect-only aliases, the legacy uppercase MBTI slug, and the non-canonical `__unknown__` Career family. The sitemap is discoverability evidence, not the sole content-authority source; backend/repository route contracts remain authoritative for canonical eligibility.
...
The supported device values are `mobile`, `desktop`, and `tablet`. Surfaces are classified from verified canonical paths as assessment, personality, articles, career, hubs/content pages, and homepage.
...
- B03 inventory exists at `authority-b03-technical-manual-evidence/technical_manual_inventory.json`, SHA-256 `bbef0a734aae19f7759e49b1dae24fca2afa25b46652777b309a8bd1ce572893`, and validates six assessments. Its status is `B03_TECHNICAL_EVIDENCE_PARTIALLY_BLOCKED`: public sample/norm, reliability, validity, reviewer, changelog and complete technical-manual evidence gaps remain `UNKNOWN` where the inventory records them.
- C06/C07 completion and a formal current Career batch remain `WAITING_ON_C06_C07`.

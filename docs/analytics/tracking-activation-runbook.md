...
For a production-mode local build whose analytics allowlist uses a synthetic hostname, expose the standalone server through a temporary local HTTPS endpoint, add `--resolve-to 127.0.0.1`, and pass that HTTPS hostname in `--base-url`. Production CSP upgrades insecure subresources, so a plain HTTP standalone origin cannot prove client hydration. The resolver option accepts only an IPv4 address or `::1`; only this explicit local mode disables proxy inheritance and accepts the temporary local certificate. The scheduled/production path does not use host remapping or relaxed certificate handling.

The schema `1.1` JSON result must report `health_status: "healthy"`, a completed real CookieBanner accept action, the real landing-pageview session marker, matching header/bootstrap/dynamic nonces, distinct nonces from two independent browser contexts, all three loader/write attempts, equal non-zero telemetry attempt/abort counts, zero CSP script-blocking errors, and private-route suppression. The default public probe path is `/`, avoiding the `/zh` redirect boundary. GA, Baidu, and `/api/track` requests are all aborted by the browser probe without reading request bodies; the report contains no measurement/site IDs, request bodies, cookies, sensitive query values, or HTML dumps.

...
| BIG5_OCEAN | `ready_readonly_cleared` | `STANDARD_ALIGNED` | `DOC_MATCH` | `Big5ResultPageV2Shell` doc match | existing backend asset agent | PDF QA and shared contracts doc match | `DOC_MATCH_CLEARED_WITH_SANITIZED_API_EVIDENCE`: historical fap-web evidence had `share_safety_missing_count=1`; sanitized fap-api PR #2326/#2331 evidence has `share_safety_missing_count=0`, `validation_error_count=0`, `leak_hit_count=0` | doc match | doc match | doc match cleared for read-only, with pilot/runtime/production still held | not generated in fap-web | `HOLD_PRODUCTION_AND_RUNTIME` | `READY_READONLY`, `READY_READONLY_CLEARED`, `STANDARD_ALIGNED`, `NEEDS_READONLY_REVIEW`, `HOLD_PRODUCTION`, `HOLD_RUNTIME`, `HOLD_CMS_SEARCH` | `BIG5-RESULT-PAGE-AGENT-READY-READONLY-CLEARED-HANDOFF-01` |
...
| BIG5_OCEAN | `BIG5-RESULT-PAGE-AGENT-READY-READONLY-CLEARED-HANDOFF-01` | Existing stack is standard-aligned and the read-only share-safety conflict is cleared by sanitized fap-api PR #2326/#2331 evidence with `share_safety_missing_count=0`, `validation_error_count=0`, and `leak_hit_count=0`. Pilot/runtime/production claims remain held. |
...
- `BIG5-READY-READONLY-CLEARED-HANDOFF`: fap-web consumes sanitized fap-api PR #2326/#2331 evidence showing `share_safety_missing_count=0`, `validation_error_count=0`, and `leak_hit_count=0`; this clears read-only evidence only and does not authorize pilot, runtime, production, CMS, search, or private result data.

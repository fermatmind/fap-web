...
Every lane manifest carries a gap-free `transition_trace` for each ordered state crossed in that Producer PR. A `qa_pass` or `dry_run_ready` manifest must bind the physical V2 W9 report bytes, report SHA, complete registered asset IDs/count, producer lane/subscope, exact package SHA, and immutable reviewed-source commit; required PR CI proves that reviewed commit is an ancestor of the current exact PR head. The complete named W9 check set and verdict must be `PASS`. Promotion states are forbidden in lane manifests and come only from the complete authenticated receipt chain present in the trusted workflow artifact. Required contract CI receives a read-only GitHub token so it can verify public cross-repository workflow runs and receipt artifacts without a user secret.

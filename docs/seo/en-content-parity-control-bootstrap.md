...
For completeness, the V2 freeze artifact uses a dedicated package root containing only `sha256_manifest.json` and its declared payloads. The validator rejects undeclared files and symlinks, byte-checks every payload, and requires the sum of actual non-empty JSONL records to equal the registered target count.

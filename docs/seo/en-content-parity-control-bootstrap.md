...
Every lane manifest carries a gap-free `transition_trace` for each ordered state crossed in that Producer PR. A new `package_frozen` transition must bind a V2 package-freeze artifact whose digest is deterministically computed from the registered lane/subscope, complete asset IDs/count, and every non-symlinked payload path/SHA; `{}` or a declared-only package SHA is never freeze evidence. A `qa_pass` or `dry_run_ready` manifest must bind the physical V2 W9 report bytes, report SHA, complete registered asset IDs/count, producer lane/subscope, exact package SHA, and immutable reviewed-source commit; required PR CI proves that reviewed producer commit is an ancestor of the current exact fap-web PR head. The complete named W9 check set and verdict must be `PASS`. `dry_run_ready` additionally requires the exact fap-api preflight receipt with its independent backend source commit, zero writes/releases/mutations, registered row-count equality, pinned policy SHA, and valid receipt content SHA. Lane manifests cannot override asset-registry counts. Promotion states are forbidden in lane manifests and come only from the complete authenticated receipt chain present in the trusted workflow artifact, whose row count must also equal the registered target. Required contract CI receives a read-only GitHub token so it can verify public cross-repository workflow runs and receipt artifacts without a user secret. Required PR validation also compares the generated authority with the base branch and rejects replacing a verified receipt state with an older prefix for the same package.

...
- Read-only observation: `2026-08-01T08:59:24.077Z`.
...
- Artifact SHA-256: `2389dd0897a61cb33508c5a5d2be873d69174b347fff59375a36d6cbd6d4d054`.
...
- current SEO authority, including `index,follow`, index eligibility, metadata fingerprint and a frozen SEO observation SHA; a dedicated SEO-endpoint payload is eligible only on endpoint HTTP 200 with a nonempty fingerprint exactly matching the reviewed detail `seo_contract`, while a valid detail fallback is explicitly labeled and counted;
...
- visible-content thickness measured from the public rendered HTML body above the fixed floor, independently of API authority payload size; crawler metadata, scripts, styles, comments, templates, `hidden`, `aria-hidden=\"true\"`, the repository `hidden` utility class, and inline `display:none` / `visibility:hidden` subtrees cannot satisfy this gate;
- explicit nonempty approved hero and definition markers plus FAQ authority markers present in the rendered body, with only marker hashes and counts frozen in the artifact; FAQ volume cannot substitute for a missing hero or definition block;

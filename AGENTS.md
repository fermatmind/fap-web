...
- Exception for a failed frozen package: when an independent W9 report bound to the exact `package_frozen` SHA has verdict `BLOCKED`, a separate CONTROL-only rework reset may move that target to `package_in_progress`. The reset must clear the failed `package_sha256`, `qa_report_ref`, and `gate_lineage`, retain the W9 report as external evidence, keep all write/release permissions false, and occur before any producer rebuild. This is not a normal blocked recovery and requires a fresh complete package freeze plus fresh independent W9 QA.

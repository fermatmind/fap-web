...
export default async function LemonSqueezySuccessPage({
  params,
  searchParams,
}: {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-10">
      <OrderReturnFallbackClient locale={locale} orderNo={orderNo || null} paymentRecoveryToken={paymentRecoveryToken} />
      <h1 className="m-0 text-2xl font-bold text-slate-900">{isZh ? "支付已完成" : "Payment Completed"}</h1>
      <p className="mt-3 text-slate-600">
        {isZh
          ? "我们正在同步订单状态。你可以前往订单查询页查看最新结果。"
          : "We're syncing your order status. You can check the latest status on the order lookup page."}
      </p>
      <Link

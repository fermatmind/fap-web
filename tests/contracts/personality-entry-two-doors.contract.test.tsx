...
describe("personality entry two doors contract", () => {
  const source = read("app/(localized)/[locale]/personality/page.tsx");

  it("keeps the personality index focused on one main action plus type browsing", () => {
    expect(source).toContain("看懂你的人格类型");
    expect(source).toContain("先找到自己的类型，再进入更适合你的内容方向。");
    expect(source).toContain("我还不知道自己的类型");
    expect(source).toContain("先做一次 MBTI 测试");
    expect(source).toContain("我已经知道自己的类型");
    expect(source).toContain("直接查看人格类型内容");
    ...
    expect(source).toContain('href="#type-groups"');
  });

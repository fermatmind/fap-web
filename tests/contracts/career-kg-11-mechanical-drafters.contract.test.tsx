...
import { isPrCareerKg11AllowedFile } from "./helpers/currentPrScope";

const PACKAGE_DIR = "generated/career-kg-pr-11-mechanical-drafters";
const ASSET_PATH = `${PACKAGE_DIR}/mechanical-drafters.zh-CN.asset.json`;
...
describe("PR-CAREER-KG-11 mechanical drafters career KG asset", () => {
  ...
  it("delivers identity, SEO, source, and block coverage without changing canonical", () => {
    expect(asset).toMatchObject({
      ...
      pr_id: "PR-CAREER-KG-11",
      status: "dry_run_ready",
      locale: "zh-CN",
      slug: "mechanical-drafters",
      canonical_path: "/zh/career/jobs/mechanical-drafters",
      canonical_unchanged: true,
      production_import_approved: false,
      staging_write_approved: false,
    });
    expect(asset.identity).toMatchObject({
      standard_name_zh: "机械制图员",
      title_en: "Mechanical Drafters",
      soc_code: "17-3013",
      onet_code: "17-3013.00",
    });
    expect(asset.identity.aliases_zh).toEqual(expect.arrayContaining(["机械 CAD 制图员", "CAD 制图员", "工程图绘图员"]));
    expect(asset.identity.easily_confused_occupations.map((item) => item.name_zh)).toEqual(
      expect.arrayContaining(["机械工程师", "机械工程技术员/技师", "工业设计师", "电气/电子制图员", "建筑/土木制图员"])
    );
    expect(asset.seo.title_zh).toContain("机械制图员是做什么的");
    expect(asset.seo.title_zh).toContain("CAD、GD&T、工程图");
    expect(asset.seo.title_zh).toContain("相邻制图职业");
    expect(asset.seo.h1_zh).toBe("机械制图员");
    ...
    expect(asset.content_blocks.find((block) => block.id === "core_responsibilities")?.items_zh?.join(" ")).toContain("工程变更单");
    expect(asset.content_blocks.find((block) => block.id === "skills_tools")?.items_zh?.join(" ")).toContain("GD&T");
    expect(asset.content_blocks.find((block) => block.id === "risk_ai_boundary")?.items_zh?.join(" ")).toContain("工程判断");
  });
  ...
  it("keeps occupational facts on O*NET/BLS/My Next Move authority", () => {
    ...
    expect(sourceKeys).toEqual(
      new Set([
        "onet_summary_17_3013_00",
        "bls_ooh_drafters",
        "bls_oews_17_3013",
        "mynextmove_summary_17_3013_00",
        "mynextmove_data_sources_17_3013_00",
        "bls_ooh_mechanical_engineering_technicians",
    ...
    expect(asset.sources.filter((source) => source.authority === "occupation_fact").map((source) => source.label)).toEqual(
      expect.arrayContaining([
        "O*NET OnLine: Mechanical Drafters 17-3013.00",
        "BLS OOH: Drafters",
        "BLS OEWS: Mechanical Drafters 17-3013",
        "My Next Move: Mechanical Drafters 17-3013.00",
      ])
    );
    expect(asset.market_reference_policy.zh_recruiting_or_baike_sources_used).toBe(false);
    expect(asset.market_reference_policy.blocked_usage).toEqual(expect.arrayContaining(["职业事实权威", "工程签字权限保证", "机械工程师转岗保证"]));
  ...
  it("passes QA, dry-run, smoke, and release boundary gates", () => {
    ...
    expect(qa.trust_audit.blocked_claims_absent).toEqual(expect.arrayContaining(["engineering_license_or_signoff_guarantee"]));
    expect(qa.editorial_gate.reader_intent_covered).toEqual(expect.arrayContaining(["机械制图员是做什么的", "GD&T 工程图"]));
  ...
  it("renders through fap-web career display primitives from a backend-like projection", () => {
    ...
    expect(screen.getByTestId("career-display-surface")).toHaveTextContent("机械制图员");
    expect(screen.getByTestId("career-display-hero")).toHaveTextContent("CAD");
    expect(screen.getByTestId("definition-block")).toHaveTextContent("尺寸");
    expect(screen.getByTestId("responsibilities-block")).toHaveTextContent("工程变更单");
    expect(screen.getByTestId("career-display-faq")).toHaveTextContent("机械工程师");
  ...
  it("records stable hashes and keeps PR scope constrained", () => {
    ...
    expect(shaManifest["mechanical-drafters.zh-CN.asset.json"]).toBe(sha256(ASSET_PATH));
    ...
    expect(manifest).toContain("PR-CAREER-KG-11");
    expect(manifest).toContain("codex/pr-career-kg-11-mechanical-drafters");
    expect(state["PR-CAREER-KG-10"]).toMatchObject({ status: "merged", merge_commit_sha: "46c10c6b9205e0d5918500ffdd728078a163efa9" });
    expect(state["PR-CAREER-KG-11"]).toMatchObject({ branch: "codex/pr-career-kg-11-mechanical-drafters" });

    const files = currentChangedFiles();
    expect(files.every((file) => isPrCareerKg11AllowedFile(file)), files.join("\n")).toBe(true);
  });
});

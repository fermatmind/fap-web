...
import { isPrCareerKg12AllowedFile } from "./helpers/currentPrScope";

const PACKAGE_DIR = "generated/career-kg-pr-12-search-marketing-strategists";
const ASSET_PATH = `${PACKAGE_DIR}/search-marketing-strategists.zh-CN.asset.json`;
...
describe("PR-CAREER-KG-12 search marketing strategists career KG asset", () => {
  ...
  it("delivers identity, SEO, source, and block coverage without changing canonical", () => {
    expect(asset).toMatchObject({
      ...
      pr_id: "PR-CAREER-KG-12",
      status: "dry_run_ready",
      locale: "zh-CN",
      slug: "search-marketing-strategists",
      canonical_path: "/zh/career/jobs/search-marketing-strategists",
      canonical_unchanged: true,
      production_import_approved: false,
      staging_write_approved: false,
    });
    expect(asset.identity).toMatchObject({
      standard_name_zh: "搜索营销策略师",
      title_en: "Search Marketing Strategists",
      soc_code: "13-1161",
      onet_code: "13-1161.01",
    });
    expect(asset.identity.aliases_zh).toEqual(expect.arrayContaining(["SEM 策略师", "SEO 策略师", "PPC 专员"]));
    expect(asset.identity.easily_confused_occupations.map((item) => item.name_zh)).toEqual(
      expect.arrayContaining(["市场研究分析师 / 营销专家", "市场经理", "广告与推广经理", "内容营销 / SEO 编辑", "数据分析师"])
    );
    expect(asset.seo.title_zh).toContain("搜索营销策略师是做什么的");
    expect(asset.seo.title_zh).toContain("SEO、SEM、数据分析");
    expect(asset.seo.title_zh).toContain("AI 自动化边界");
    expect(asset.seo.h1_zh).toBe("搜索营销策略师");
    ...
    expect(asset.content_blocks.find((block) => block.id === "core_responsibilities")?.items_zh?.join(" ")).toContain("质量分");
    expect(asset.content_blocks.find((block) => block.id === "skills_tools")?.items_zh?.join(" ")).toContain("ROAS");
    expect(asset.content_blocks.find((block) => block.id === "risk_ai_boundary")?.items_zh?.join(" ")).toContain("保证排名");
  });
  ...
  it("keeps occupational facts on O*NET/BLS/My Next Move authority", () => {
    ...
    expect(sourceKeys).toEqual(
      new Set([
        "onet_summary_13_1161_01",
        "bls_ooh_market_research_analysts",
        "bls_oews_13_1161",
        "mynextmove_summary_13_1161_01",
        "mynextmove_data_sources_13_1161_01",
    ...
    expect(asset.sources.filter((source) => source.authority === "occupation_fact").map((source) => source.label)).toEqual(
      expect.arrayContaining([
        "O*NET OnLine: Search Marketing Strategists 13-1161.01",
        "BLS OOH: Market Research Analysts",
        "BLS OEWS: Market Research Analysts and Marketing Specialists 13-1161",
        "My Next Move: Search Marketing Strategists 13-1161.01",
      ])
    );
    expect(asset.market_reference_policy.zh_recruiting_or_baike_sources_used).toBe(false);
    expect(asset.market_reference_policy.blocked_usage).toEqual(expect.arrayContaining(["职业事实权威", "排名保证", "ROI 保证"]));
  ...
  it("passes QA, dry-run, smoke, and release boundary gates", () => {
    ...
    expect(qa.trust_audit.blocked_claims_absent).toEqual(expect.arrayContaining(["ranking_guarantee", "traffic_guarantee", "roi_guarantee"]));
    expect(qa.editorial_gate.reader_intent_covered).toEqual(expect.arrayContaining(["搜索营销策略师是做什么的", "AI 自动化边界"]));
  ...
  it("renders through fap-web career display primitives from a backend-like projection", () => {
    ...
    expect(screen.getByTestId("career-display-surface")).toHaveTextContent("搜索营销策略师");
    expect(screen.getByTestId("career-display-hero")).toHaveTextContent("搜索引擎优化");
    expect(screen.getByTestId("definition-block")).toHaveTextContent("付费搜索");
    expect(screen.getByTestId("responsibilities-block")).toHaveTextContent("质量分");
    expect(screen.getByTestId("career-display-faq")).toHaveTextContent("AI");
  ...
  it("records stable hashes and keeps PR scope constrained", () => {
    ...
    expect(shaManifest["search-marketing-strategists.zh-CN.asset.json"]).toBe(sha256(ASSET_PATH));
    ...
    expect(manifest).toContain("PR-CAREER-KG-12");
    expect(manifest).toContain("codex/pr-career-kg-12-search-marketing-strategists");
    expect(state["PR-CAREER-KG-11"]).toMatchObject({ status: "merged", merge_commit_sha: "3a2429e86a6f5ad079c0f319a3ee64a21a9cfb1a" });
    expect(state["PR-CAREER-KG-12"]).toMatchObject({ branch: "codex/pr-career-kg-12-search-marketing-strategists" });

    const files = currentChangedFiles();
    expect(files.every((file) => isPrCareerKg12AllowedFile(file)), files.join("\n")).toBe(true);
  });
});

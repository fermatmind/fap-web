...
const SCRIPT = "scripts/seo/build-mbti-profile-nt-31-content-package.mjs";
const OUTPUT = "docs/seo/personality/mbti-profile-nt-31-content-package-2026-07-13.json";
const EXPECTED_PATHS = [
  "/zh/personality/intj-a", "/zh/personality/intj-t", "/zh/personality/intp-a", "/zh/personality/intp-t",
  "/zh/personality/entj-a", "/zh/personality/entj-t", "/zh/personality/entp-a", "/zh/personality/entp-t",
];
...
describe("MBTI-PROFILE-NT-31 content package", () => {
  it("builds exactly eight Chinese NT profile candidates", () => {
    const stdout = execFileSync("node", [SCRIPT], { cwd: ROOT, encoding: "utf8" });
    expect(JSON.parse(stdout)).toMatchObject({ ok: true, artifact: "MBTI-PROFILE-NT-31-CONTENT-PACKAGE", target_count: 8 });
    const report = readPackage();
    expect(report.final_decision).toBe("PASS_NON_PRODUCTION_NT_PROFILE_CONTENT_PACKAGE_READY_FOR_FULL_QA");
    expect(report.summary).toMatchObject({ target_count: 8, group: "NT" });

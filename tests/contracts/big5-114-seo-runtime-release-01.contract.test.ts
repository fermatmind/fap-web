...
describe("BIG5-114-SEO-RUNTIME-RELEASE-01", () => {
  it("accepts exactly 114 backend-authorized canonical Big Five routes and rejects redirect aliases", () => {
    const aliases = Object.keys(BIG_FIVE_ZH_LEGACY_TO_V2_SLUG).map(
      (slug) => `https://fermatmind.com/zh/personality/big-five/${slug}`
    );

...
import { isPrCareerKg04AllowedFile } from "./helpers/currentPrScope";

const PACKAGE_DIR = "generated/career-kg-pr-04-archivists";
const ASSET_PATH = `${PACKAGE_DIR}/archivists.zh-CN.asset.json`;
...
describe("PR-CAREER-KG-04 archivists career KG asset", () => {
  ...
  it("delivers the required identity, SEO, source, and content-block package without changing canonical", () => {
    expect(asset).toMatchObject({
      ...
      pr_id: "PR-CAREER-KG-04",
      status: "dry_run_ready",
      locale: "zh-CN",
      slug: "archivists",
      canonical_path: "/zh/career/jobs/archivists",
      canonical_unchanged: true,
      production_import_approved: false,
      staging_write_approved: false,
    });
    expect(asset.identity).toMatchObject({
      standard_name_zh: "档案管理员",
      title_en: "Archivists",
      soc_code: "25-4011",
      onet_code: "25-4011.00",
    });
    expect(asset.identity.aliases_zh).toEqual(
      expect.arrayContaining(["数字档案管理员", "档案编目员", "档案检索员", "记录管理员"])
    );
    expect(asset.identity.aliases_en).toEqual(
      expect.arrayContaining(["Digital Archivist", "Reference Archivist", "Records Manager", "Registrar"])
    );
    expect(asset.identity.easily_confused_occupations.map((item) => item.name_zh)).toEqual(
      expect.arrayContaining(["图书馆员", "博物馆馆长/策展人", "档案文员/文件管理员", "记录管理员"])
    );

    expect(asset.seo.title_zh).toContain("档案管理员是做什么的");
    expect(asset.seo.title_zh).toContain("分类编目");
    expect(asset.seo.title_zh).toContain("数字档案管理");
    expect(asset.seo.meta_description_zh).toContain("档案接收");
    expect(asset.seo.meta_description_zh).toContain("记录管理");
    expect(asset.seo.h1_zh).toBe("档案管理员");
    expect(asset.seo.canonical_path).toBe("/zh/career/jobs/archivists");
    ...
    expect(asset.content_blocks.find((block) => block.id === "skills_tools")?.items_zh?.join(" ")).toContain("元数据");
    expect(asset.content_blocks.find((block) => block.id === "core_responsibilities")?.items_zh?.join(" ")).toContain("检索");
  ...
  it("keeps facts on O*NET/BLS/My Next Move authority and quarantines market/search references", () => {
    ...
    expect(sourceKeys).toEqual(
      new Set([
        "onet_summary_25_4011_00",
        "bls_ooh_archivists_curators_museum_workers",
        "mynextmove_summary_25_4011_00",
        "mynextmove_data_sources_25_4011_00",
    ...
    expect(asset.sources.filter((source) => source.authority === "occupation_fact").map((source) => source.label)).toEqual(
      expect.arrayContaining([
        "O*NET OnLine: Archivists 25-4011.00",
        "BLS OOH: Archivists, Curators, and Museum Workers",
        "My Next Move: Archivists 25-4011.00",
      ])
    );
  ...
  it("passes schema validation, trust audit, editorial gate, dry-run importer, and smoke gates", () => {
    ...
    expect(qa.editorial_gate).toMatchObject({
      status: "pass",
      canonical_path_locked: "/zh/career/jobs/archivists",
    });
    expect(qa.editorial_gate.reader_intent_covered).toEqual(
      expect.arrayContaining(["档案管理员是做什么的", "档案分类编目", "数字档案管理", "档案管理员和图书馆员区别"])
    );
    ...
    expect(renderSmoke).toMatchObject({
      ...
      route: "/zh/career/jobs/archivists",
    });
  ...
  it("renders through fap-web career display primitives from a backend-like projection", () => {
    ...
    expect(screen.getByTestId("career-display-surface")).toHaveTextContent("档案管理员");
    expect(screen.getByTestId("career-display-hero")).toHaveTextContent("永久记录");
    expect(screen.getByTestId("definition-block")).toHaveTextContent("检索工具");
    expect(screen.getByTestId("responsibilities-block")).toHaveTextContent("分类体系");
    expect(screen.getByTestId("career-display-faq")).toHaveTextContent("档案管理员和图书馆员有什么区别");
  ...
  it("records stable SHA-256 hashes and keeps PR scope constrained", () => {
    ...
    expect(shaManifest["archivists.zh-CN.asset.json"]).toBe(sha256(ASSET_PATH));
    ...
    expect(manifest).toContain("PR-CAREER-KG-04");
    expect(manifest).toContain("codex/pr-career-kg-04-archivists");
    expect(state["PR-CAREER-KG-03"]).toMatchObject({
      status: "merged",
      merge_commit_sha: "b4f051dc4c5d42ae8c9206a1d39ee09db44fbc69",
    });
    expect(state["PR-CAREER-KG-04"]).toMatchObject({
      branch: "codex/pr-career-kg-04-archivists",
    });

    const files = currentChangedFiles();
    expect(files.every((file) => isPrCareerKg04AllowedFile(file)), files.join("\n")).toBe(true);
  });
});

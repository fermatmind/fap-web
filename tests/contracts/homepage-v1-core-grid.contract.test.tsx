...
describe("homepage v1 core grid contract", () => {
  it("renders exactly six core test cards while removing red-boxed grid intro and meta text", () => {
    render(<HomePageExperience locale="zh" />);
    const firstCardTitle = screen.getByRole("heading", { level: 3, name: "MBTI 性格测试" });
    const section = firstCardTitle.closest("section");
    ...
    expect(screen.queryByRole("heading", { level: 2, name: "从一个清楚的问题开始。" })).not.toBeInTheDocument();
    expect(screen.queryByText("人格测试")).not.toBeInTheDocument();
    expect(screen.queryByText("能力测评")).not.toBeInTheDocument();
    expect(screen.queryByText("情绪能力")).not.toBeInTheDocument();
    expect(screen.queryByText("状态自测")).not.toBeInTheDocument();
  });

...
const SCRIPT = "scripts/seo/build-mbti-profile-sp-34-content-package.mjs";
const OUTPUT = "docs/seo/personality/mbti-profile-sp-34-content-package-2026-07-13.json";
const EXPECTED_PATHS = [
  "/zh/personality/istp-a", "/zh/personality/istp-t", "/zh/personality/isfp-a", "/zh/personality/isfp-t",
  "/zh/personality/estp-a", "/zh/personality/estp-t", "/zh/personality/esfp-a", "/zh/personality/esfp-t",
];
...
describe("MBTI-PROFILE-SP-34 content package", () => {
  it("builds exactly eight Chinese SP profile candidates", () => {
    const stdout = execFileSync("node", [SCRIPT], { cwd: ROOT, encoding: "utf8" });
    expect(JSON.parse(stdout)).toMatchObject({ ok: true, artifact: "MBTI-PROFILE-SP-34-CONTENT-PACKAGE", target_count: 8 });
    const report = readPackage();
    expect(report.final_decision).toBe("PASS_NON_PRODUCTION_SP_PROFILE_CONTENT_PACKAGE_READY_FOR_FULL_QA");
    expect(report.summary).toMatchObject({ target_count: 8, group: "SP", repair_count: 6, verify_only_count: 2 });
  ...
  it("repairs six profiles and verifies two existing assets without proposing an unjustified rewrite", () => {
    ...
    expect(verifyOnlyAssets.map((asset) => asset.path)).toEqual(["/zh/personality/istp-a", "/zh/personality/isfp-a"]);

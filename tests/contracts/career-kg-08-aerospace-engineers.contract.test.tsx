...
import { isPrCareerKg08AllowedFile } from "./helpers/currentPrScope";

const PACKAGE_DIR = "generated/career-kg-pr-08-aerospace-engineers";
const ASSET_PATH = `${PACKAGE_DIR}/aerospace-engineers.zh-CN.asset.json`;
...
describe("PR-CAREER-KG-08 aerospace engineers career KG asset", () => {
  ...
  it("delivers the required identity, SEO, source, and content-block package without changing canonical", () => {
    expect(asset).toMatchObject({
      ...
      pr_id: "PR-CAREER-KG-08",
      status: "dry_run_ready",
      locale: "zh-CN",
      slug: "aerospace-engineers",
      canonical_path: "/zh/career/jobs/aerospace-engineers",
      canonical_unchanged: true,
      production_import_approved: false,
      staging_write_approved: false,
    });
    expect(asset.identity).toMatchObject({
      standard_name_zh: "航空航天工程师",
      title_en: "Aerospace Engineers",
      soc_code: "17-2011",
      onet_code: "17-2011.00",
    });
    expect(asset.identity.aliases_zh).toEqual(expect.arrayContaining(["航空工程师", "航天工程师", "飞控工程师"]));
    expect(asset.identity.aliases_en).toEqual(
      expect.arrayContaining(["Aerospace Engineer", "Aeronautical Engineer", "Flight Test Engineer", "Systems Engineer"])
    );
    expect(asset.identity.easily_confused_occupations.map((item) => item.name_zh)).toEqual(
      expect.arrayContaining(["航空航天工程与运营技术员", "飞机与航空电子设备维修技师", "机械工程师", "电气与电子工程师", "飞行员"])
    );

    expect(asset.seo.title_zh).toContain("航空航天工程师是做什么的");
    expect(asset.seo.title_zh).toContain("系统设计");
    expect(asset.seo.title_zh).toContain("仿真");
    expect(asset.seo.title_zh).toContain("测试");
    expect(asset.seo.title_zh).toContain("法规");
    expect(asset.seo.meta_description_zh).toContain("教育路径");
    expect(asset.seo.h1_zh).toBe("航空航天工程师");
    ...
    expect(asset.content_blocks.find((block) => block.id === "core_responsibilities")?.items_zh?.join(" ")).toContain("仿真模型");
    expect(asset.content_blocks.find((block) => block.id === "skills_tools")?.items_zh?.join(" ")).toContain("MATLAB");
    expect(asset.content_blocks.find((block) => block.id === "risk_ai_boundary")?.items_zh?.join(" ")).toContain("法规合规");
  });
  ...
  it("keeps facts on O*NET/BLS/My Next Move authority and quarantines market/search references", () => {
    ...
    expect(sourceKeys).toEqual(
      new Set([
        "onet_summary_17_2011_00",
        "bls_ooh_aerospace_engineers",
        "mynextmove_summary_17_2011_00",
        "mynextmove_data_sources_17_2011_00",
    ...
    expect(asset.sources.filter((source) => source.authority === "occupation_fact").map((source) => source.label)).toEqual(
      expect.arrayContaining([
        "O*NET OnLine: Aerospace Engineers 17-2011.00",
        "BLS OOH: Aerospace Engineers",
        "My Next Move: Aerospace Engineers 17-2011.00",
      ])
    );
    ...
    expect(asset.market_reference_policy.blocked_usage).toEqual(
      expect.arrayContaining(["职业事实权威", "就业保证", "安全许可保证", "JSON-LD事实来源", "生产导入依据"])
    );
  ...
  it("passes schema validation, trust audit, editorial gate, dry-run importer, and smoke gates", () => {
    ...
    expect(qa.trust_audit.blocked_claims_absent).toEqual(expect.arrayContaining(["security_clearance_guarantee"]));
    expect(qa.editorial_gate).toMatchObject({
      status: "pass",
      canonical_path_locked: "/zh/career/jobs/aerospace-engineers",
    });
    expect(qa.editorial_gate.reader_intent_covered).toEqual(
      expect.arrayContaining(["航空航天工程师是做什么的", "航空航天工程师 系统设计", "仿真分析", "原型测试"])
    );
    ...
    expect(renderSmoke).toMatchObject({
      ...
      route: "/zh/career/jobs/aerospace-engineers",
    });
  ...
  it("renders through fap-web career display primitives from a backend-like projection", () => {
    ...
    expect(screen.getByTestId("career-display-surface")).toHaveTextContent("航空航天工程师");
    expect(screen.getByTestId("career-display-hero")).toHaveTextContent("系统设计");
    expect(screen.getByTestId("definition-block")).toHaveTextContent("飞机");
    expect(screen.getByTestId("definition-block")).toHaveTextContent("航天器");
    expect(screen.getByTestId("responsibilities-block")).toHaveTextContent("仿真");
    expect(screen.getByTestId("career-display-faq")).toHaveTextContent("航空航天工程师需要什么学历");
  ...
  it("records stable SHA-256 hashes and keeps PR scope constrained", () => {
    ...
    expect(shaManifest["aerospace-engineers.zh-CN.asset.json"]).toBe(sha256(ASSET_PATH));
    ...
    expect(manifest).toContain("PR-CAREER-KG-08");
    expect(manifest).toContain("codex/pr-career-kg-08-aerospace-engineers");
    expect(state["PR-CAREER-KG-07"]).toMatchObject({
      status: "merged",
      merge_commit_sha: "6b0cb454c7e8087bd7efb800e814a9229f88b093",
    });
    expect(state["PR-CAREER-KG-08"]).toMatchObject({
      branch: "codex/pr-career-kg-08-aerospace-engineers",
    });

    const files = currentChangedFiles();
    expect(files.every((file) => isPrCareerKg08AllowedFile(file)), files.join("\n")).toBe(true);
  });
});

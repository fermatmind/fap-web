...
test("home page renders hero, value props, and highlighted tests", async ({ page }) => {
  ...
  await expect(highlightedSection.locator('a[href*="/take"]')).toHaveCount(5);
...
test("zh home MBTI highlighted card title shows 16型人格", async ({ page }) => {
  ...
  await expect(highlightedSection.getByRole("link", { name: "抑郁焦虑综合检测", exact: true })).toHaveCount(0);

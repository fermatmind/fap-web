...
if (!Number.isInteger(detectedNodeMajor) || detectedNodeMajor !== REQUIRED_NODE_MAJOR) {
  fail([
    ...
    `[check-runtime] This repository requires canonical Node.js ${REQUIRED_NODE_MAJOR}.x.`,
    "[check-runtime] Run `nvm use` (or switch to Node 24.x) and try again.",
  ]);
}
...
if (packageManagerUserAgent) {
  console.log(`[check-runtime] OK: canonical Node ${REQUIRED_NODE_MAJOR}.x (${detectedNodeVersion}) with ${packageManagerUserAgent}`);
} else {
  console.log(`[check-runtime] OK: canonical Node ${REQUIRED_NODE_MAJOR}.x (${detectedNodeVersion})`);
}

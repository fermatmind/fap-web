...
export function validateW4RiasecW9Qa() {
  ...
  if (resetMode) {
    ...
    assert(
      reset.$schema === "docs/seo/generated/en-content-parity-control-master.v1.schema.json" &&
        ...
        reset.blocked_package_sha256 === packageSha &&
  ...
  const mapIds = resetMode
    ? projection.atomic_row_identity_projection.map((row) => row.row_id)
    : map.atomic_rows.map((row) => row.row_id);
  if (!resetMode) {
  ...
  assert(
    repairPlan.package_sha256 === packageSha &&
  ...
  return { ok: true, package_sha256: packageSha, rows: 1550, blocked_rows: 130, language_blocked_rows: 126, duplicate_blocked_rows: 4, verdict: "BLOCKED", control_reset: resetMode };
}

...
def main():
    ...
    for ev in read_jsonl(a.evidence):
        ...
        guidance=[{'title':('先做兴趣验证' if loc=='zh-CN' else 'start with interest validation'),'body':('用 RIASEC/霍兰德看你是否愿意长期处理这些任务材料和责任结构。' if loc=='zh-CN' else 'Use RIASEC/Holland-style testing to check whether you can sustain interest in these task materials and responsibility patterns.')},{'title':('再做工作样本验证' if loc=='zh-CN' else 'then test with work samples'),'body':('选一个小型任务样本，记录你对材料、节奏、协作和复核压力的真实反应。' if loc=='zh-CN' else 'Try a small work sample and record how you respond to the materials, pace, collaboration, and review pressure.')}]

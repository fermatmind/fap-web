...
OUTCOME_CLAIMS=re.compile(r"\b(will get hired|job offer|salary|wage|income|promotion guaranteed|guaranteed promotion|admission guaranteed|guaranteed admission|certification guaranteed|employment guaranteed|guarantee[sd]?)\b|保证(?:就业|入职|录取|拿证|收入|薪资|工资|升职)", re.I)
...
def _allowed_occupational_outcome_context(sentence: str) -> bool:
    lowered = sentence.lower()
    return any(marker in lowered for marker in ("property income stream", "anticipated property income", "income capitalization", "income potential", "property values", "property value", "low-income population", "low income population", "employee retirement income security act", "compensation polic", "compensation plan", "prevailing wage rate", "available monthly income", "income growth", "financial situations", "financial information", "debt obligations", "credit data", "extending credit", "lending money", "loan requests", "credit reports", "profitability of loans", "financial solvency", "financial integrity", "financial conditions", "operating income and expense accounts", "loan documentation", "loan investment portfolios", "capital, earnings", "pay adjustments", "employee paychecks", "statements of earnings", "compute wages", "wage computation", "payroll system", "issued payrolls", "current income, expenses", "financial objectives", "financial goals", "financial plan", "financial plans", "financial performance", "cash management", "taxable income", "income statements", "tax returns", "tax return", "taxes owed", "clients' taxes", "deductible expenses", "薪资与考勤文员", "工时、薪酬项目、扣款和工资单记录"))

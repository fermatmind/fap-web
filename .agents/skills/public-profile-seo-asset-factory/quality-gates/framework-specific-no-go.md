# Framework-Specific No-Go

## MBTI

- Do not regenerate 64 A/T pages.
- Do not say A/T is official MBTI.
- Do not replace `PersonalityController` or CMS authority.

## Big Five

- Big Five is dimensional, not a fixed type model.
- Do not generate official 32 types.
- Do not create 32 OCEAN SEO pages.
- High/low poles are not good/bad judgments.
- Facet stubs must not index.

## Enneagram

- Lock the current estate to 58 identities and 116 pages: 1 hub, 3 centers, 9 core types, 18 wings, and 27 instinctual subtypes in each of 2 locales.
- Do not create new URLs.
- Do not create the 54 wing × instinct matrix; it is distinct from the 27 current subtype identities.
- Do not create Tritype.
- Do not frame Enneagram as clinical diagnosis or hiring basis.
- Do not represent model/agent QA as a historical human review; new V2 packages require independent W9/QA rather than `pending_manual_review`.
